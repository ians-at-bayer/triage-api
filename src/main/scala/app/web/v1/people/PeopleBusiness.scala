...
class PeopleBusiness(peopleDao: PeopleDao) {
  def createPeople(teamId: Int, peopleToCreate: Seq[PersonDTO]): Unit = {
    ...
    peopleToCreate.foreach(person => {
      if (peopleDao.loadPersonByUserId(person.userId).isDefined)
        throw new IllegalArgumentException(s"Person '${person.userId}' already exists")

      peopleDao.insertPerson(person.name, person.userId, teamId)
    })
  }
}
